...
def _select_nn_indices(
    ...
    Metrics:
      ...
      - 'lorentz'/'hyperbolic' : **geodesic distance on Lorentz model**
                                 d = arcosh(-<x,y>_L), where
                                 <x,y>_L = -x0*y0 + sum_{i>=1} x_i*y_i
    treat_as controls whether we maximize (similarity) or minimize (distance):
      - 'auto'       : cosine/dot -> maximize; others (l1/l2/lorentz) -> minimize
      - 'similarity' : maximize the score (we'll negate distances internally)
    ...
    elif metric in ("lorentz", "hyperbolic"):
        D = manifold.pairwise_distance(Q, R)  # (B, M)
    else:
        raise ValueError(f"Unknown metric '{metric}'. Use cosine, dot, euclidean/l2, manhattan/l1, lorentz/hyperbolic.")

    if treat_as == "similarity":
        idx = (-D).argmax(dim=1)  # equivalent to maximizing negative geodesic distance
    else:  # 'distance' or 'auto' (for lorentz we want nearest neighbor)
        idx = D.argmin(dim=1)
    return idx
...
def evaluate_knn1(
    ...
    device: str = "cuda",
    l2_normalize: bool = True,
    metric: str = "lorentz",   # 'cosine' | 'dot' | 'euclidean'/'l2' | 'manhattan'/'l1' | 'lorentz'/'hyperbolic'
    treat_as: str = "auto",   # 'auto' | 'similarity' | 'distance'
    manifold = None,
    ...
    - If metric='cosine', you can pass l2_normalize=True for fastest path
      (dot product == cosine).
    - For 'lorentz'/'hyperbolic' metrics, **do not L2-normalize** embeddings.
    ...
    # If using Lorentz geometry, force-disable Euclidean L2 normalization
    if metric.lower() in ("lorentz", "hyperbolic"):
        l2_normalize = False
    ...
    for xb, yb, *rest in tqdm(qry_loader, total=len(qry_loader), desc="Evaluating 1-NN"):
        xb = xb.to(device, non_blocking=True)
        Zq = model(xb)
        # Avoid L2-normalizing for Lorentz; keep for others when requested
        if l2_normalize and metric.lower() != "cosine":
        ...
        # 1-NN selection
        nn_idx = _select_nn_indices(Zq, R, metric=metric, treat_as=treat_as, manifold=manifold)  # (B,)
