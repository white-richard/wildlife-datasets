...
class SeaTurtleID2022(DownloadKaggle, WildlifeDataset):
    ...
    def create_catalogue(self, category_name="head") -> pd.DataFrame:
        ...
        if category_name not in categories:
            ...
            raise Exception(
                f"Category {category_name} not allowed. Choose one from {categories.keys()}."
            )
        ...
        create_dict = lambda i: {
            ...
            "orientation": (
                i["attributes"][orientation_col]
                if orientation_col in i["attributes"]
                else np.nan
            ),
        }
        ...
        df_annotation.loc[idx_bbox, "bbox"] = df_annotation.loc[idx_bbox, "bbox"].apply(
        ...
        df["path"] = (
            ...
            + "data"
            + os.path.sep
            + df["file_name"].str.replace("/", os.path.sep)
        )
        df = df.drop(
            [
                "id",
...
class SeaTurtleIDHeads(DownloadKaggle, WildlifeDataset):
    ...
    def create_catalogue(self) -> pd.DataFrame:
        ...
        df["path"] = (
            "images" + os.path.sep + df["identity"] + os.path.sep + df["file_name"]
        )
