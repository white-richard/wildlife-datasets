...
def train(
    ...
    eval_warmup_frac=0.60, # start validation after 75% of epochs
...
def main():
    ...
    #     T.RandomErasing(p=0.25, scale=(0.02, 0.2), ratio=(0.3, 3.3), value=0.0, inplace=True),
    # ])
    train_tfms = T.Compose([
        T.Resize((dim,dim)),
        T.ToTensor(),
    ...
    optimizer = torch.optim.SGD(
        [
            {"params": backbone_params, "weight_decay": wd},
